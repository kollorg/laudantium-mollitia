# @kollorg/laudantium-mollitia

[![CI](https://github.com/kollorg/laudantium-mollitia/workflows/CI/badge.svg)](https://github.com/kollorg/laudantium-mollitia/actions/workflows/ci.yml)
[![NPM version](https://img.shields.io/npm/v/@kollorg/laudantium-mollitia.svg?style=flat)](https://www.npmjs.com/package/@kollorg/laudantium-mollitia)
...
```
npm i @kollorg/laudantium-mollitia
```

## Compatibility with @fastify/multipart
`@kollorg/laudantium-mollitia` and [`@fastify/multipart`](https://github.com/fastify/fastify-multipart) should not be registered as sibling plugins nor should they be registered in plugins which have a parent-child relationship.`<br>` The two plugins are incompatible, in the sense that the behavior of `@kollorg/laudantium-mollitia` might not be the expected one when the above-mentioned conditions are not respected.`<br>` This is due to the fact that `@fastify/multipart` consumes the multipart content by parsing it, hence this content is not forwarded to the target service by `@kollorg/laudantium-mollitia`.`<br>`
...
proxy.register(require('@kollorg/laudantium-mollitia'), {
